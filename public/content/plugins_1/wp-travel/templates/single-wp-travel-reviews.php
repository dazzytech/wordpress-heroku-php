	...
	<div id="comments">		
		...
		<?php if ( have_comments() ) : ?>
			...
			<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
				...
				paginate_comments_links( apply_filters( 'wp_travel_comment_pagination_args', array(
					'prev_text' => '&larr;',
					'next_text' => '&rarr;',
					'type'      => 'list',
				) ) );
...
function wp_travel_comments( $comment, $args, $depth ) {
	...
	wp_travel_load_template( 'review.php', array( 'comment' => $comment, 'args' => $args, 'depth' => $depth ) );
}